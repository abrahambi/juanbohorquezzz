...
import { Mesh, Vector3, Euler, Group, ShaderMaterial, LinearSRGBColorSpace, PlaneGeometry, MeshBasicMaterial, DoubleSide, Color } from "three";
...
const createColombianFlag = (): Group => {
  const flagGroup = new Group();
  const flagWidth = 0.06;
  const stripeHeight = 0.012;

  // Yellow stripe (top, larger - 50% of flag)
  const yellowGeo = new PlaneGeometry(flagWidth, stripeHeight * 2);
  const yellowMat = new MeshBasicMaterial({ color: new Color(0xFCD116), side: DoubleSide });
  const yellow = new Mesh(yellowGeo, yellowMat);
  yellow.position.y = stripeHeight * 0.75;
  flagGroup.add(yellow);

  // Blue stripe (middle - 25%)
  const blueGeo = new PlaneGeometry(flagWidth, stripeHeight);
  const blueMat = new MeshBasicMaterial({ color: new Color(0x003893), side: DoubleSide });
  const blue = new Mesh(blueGeo, blueMat);
  blue.position.y = -stripeHeight * 0.5;
  flagGroup.add(blue);

  // Red stripe (bottom - 25%)
  const redGeo = new PlaneGeometry(flagWidth, stripeHeight);
  const redMat = new MeshBasicMaterial({ color: new Color(0xCE1126), side: DoubleSide });
  const red = new Mesh(redGeo, redMat);
  red.position.y = -stripeHeight * 1.5;
  flagGroup.add(red);
...
const setupMesh = () => {
  ...
  mesh.traverse((child) => {
    if (child instanceof Mesh) {
      ...
      // Add Colombian flag to the shirt (right chest area)
      if (child.name === "black") {
        const flag = createColombianFlag();
        flag.position.set(-0.08, 0.12, 0.15);
        flag.rotation.set(0.15, 0, 0);
        child.add(flag);
      }
    }
  });
